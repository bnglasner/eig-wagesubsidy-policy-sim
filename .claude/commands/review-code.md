---
description: Run the code error review on a target project: spawns the code-reviewer sub-agent and writes code-error-report.html to <target-dir>/review-reports/.
...
Run the shared pipeline-detection procedure in `.claude/rules/review-pipeline-runner.md`, substituting `<review-type>` = "code review".
