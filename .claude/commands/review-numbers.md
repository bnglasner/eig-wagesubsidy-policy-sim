---
description: Run the document number verification review on a target project: spawns the data-consistency-reviewer sub-agent and writes doc-number-report.html to <target-dir>/review-reports/.
...
Run the shared pipeline-detection procedure in `.claude/rules/review-pipeline-runner.md`, substituting `<review-type>` = "number verification review".
