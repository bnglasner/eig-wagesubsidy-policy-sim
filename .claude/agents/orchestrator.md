---
name: orchestrator
description: Top-level coordinator that routes work across review, writing, style, and maintenance specialists per Infrastructure/AI_WORKFLOW.md. Decides which specialist subagent to invoke, tracks dependencies, and sequences multi-stage reviews. Use when a request spans multiple specialist areas or when planning a /full-review pass.
tools: Read,Write,Edit,Bash,Glob,Grep,Task,TaskCreate,TaskUpdate,TaskList
---
...
3. `.claude/rules/plan-first-workflow.md`
4. `.claude/rules/verification-protocol.md`
5. `.claude/rules/performance-cost-governance.md`
6. Relevant specialist agent files under `.claude/agents/`
7. Relevant command playbooks under `.claude/commands/`
...
1. A proposed run exceeds the thresholds in `.claude/rules/performance-cost-governance.md`.
...
2. A dependency-aware execution plan using `.claude/templates/orchestration-plan.md`.
