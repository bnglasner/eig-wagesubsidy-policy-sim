...
Active stages depend on the project scope tier set in `Infrastructure/docs/PIPELINE_LANGUAGE_PROFILE.md`.
...
as the primary run-all control surface (see `Infrastructure/AGENTS.md` non-negotiables).
...
4. If the script produces required outputs, update `Infrastructure/docs/STAGE_OUTPUT_CONTRACTS.md`.

## Config Bootstrap

Every pipeline script must source the project config at the top:

- R: `source("code/00_setup/00_config.R")`
- Python: `import code.setup.config as cfg` (or equivalent relative import)
- Stata: `do "$path_code/00_setup/00_config.do"`

See `Infrastructure/docs/PIPELINE_LANGUAGE_PROFILE.md` for the active language profile.
