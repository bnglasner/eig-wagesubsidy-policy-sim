...
#
# Modelled on EIG IPUMS Example / code/01_data_preparation/01a_cps_pull.R
#
# Variables requested
# -------------------
...
# Output
# ------
#   data/external/asec_persons_{YYYY}.parquet
...
# Usage
# -----
#   Rscript code/01_data_preparation/02a_asec_pull.R
...
# ── Locate project root ───────────────────────────────────────────────────────
# Walks up the directory tree until it finds app/app.py.
# Override by setting EIG_WAGESUBSIDY_ROOT env var.

find_project_root <- function(
    start  = getwd(),
    marker = file.path("app", "app.py"),
    max_up = 10L
) {
...
OUTPUT_DIR <- file.path(path_project, "data", "external")
...
message(
  ...
  "\nNext step: python code/01_data_preparation/01d_asec_preprocess.py"
)
