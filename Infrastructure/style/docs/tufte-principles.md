...
> **EIG use note.** This is the design-quality (graphical-excellence) layer of the EIG
> style system. It is paired with `tufte-analytical-design.md` and applied by the
> `eig-tufte-critique` skill and the Pass 2 quality check in `eig-style-review`. It
> complements — does not replace — EIG's brand-compliance rules in
> `eig-figure-style.md` and `agent-01-design-spec.md`.
>
> **Where EIG and Tufte interact (binding):**
> - **Color.** EIG's brand palette governs *which* colors are allowed (see
>   `eig-figure-style.md` §1). Tufte governs *how many* — prefer one or two series colors
>   plus neutral gray; do not spend six brand colors where two plus gray carry the
>   comparison. "Grayscale by default" here means *minimize the number of encoded hues*,
>   not abandon the brand palette.
> - **Data density / sizing.** EIG's standard static figure is 6.5 × 3.5 in
>   (`agent-01-design-spec.md` §3.2). Small-multiple arrays and high-density displays may
>   exceed this; treat it as a default, not a ceiling, and note the exception.
>
> Source principles adapted from Edward Tufte, *The Visual Display of Quantitative
> Information*, via the `tufte-viz-claude-code` skill (Ben Glasner Style Guide).
