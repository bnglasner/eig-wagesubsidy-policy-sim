...
> **EIG use note.** Load this alongside `tufte-principles.md` when designing dashboards,
> dense displays, sparklines, small multiples, or explanatory/results graphics
> (event-study plots, coefficient plots, panel data). It is applied by the
> `eig-tufte-critique` skill. The color and sizing reconciliations stated at the top of
> `tufte-principles.md` apply here too: EIG brand palette governs *which* colors; Tufte
> governs *how many*, and the 6.5 × 3.5 in standard is a default that small-multiple
> arrays may exceed.
>
> Adapted from Edward Tufte, *Envisioning Information* (1990), *Visual Explanations*
> (1997), and *Beautiful Evidence* (2006), via the `tufte-viz-claude-code` skill
> (Ben Glasner Style Guide).
...
5. **Thoroughly describe the evidence**
   ...
   (EIG note: this is the source-line / note-line requirement in `eig-figure-style.md` §4.)
...
(EIG note: items 6–8 map directly onto EIG's "regression/event-study plots" output family
in `agent-01-design-spec.md` §6 — range-frames for coefficient axes, annotation of the
treatment moment, and small multiples through time for dynamic effects.)
