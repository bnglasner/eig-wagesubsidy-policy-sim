...
|------|------|-------------|
| `data/processed/hourly_workers.parquet` | ~330 KB | Eligible workers (output of `01a_data_ingest.py`) |
| `output/data/intermediate_results/population/*.parquet` | ~40 KB | 5 population-level summary files (output of `02a_descriptive_stats.py`) |
| `output/data/intermediate_results/individual_schedules/*.parquet` | ~3.7 MB | 204 PolicyEngine budget schedules (4 family types × 51 states) |
...
2. Commit the updated parquet files:
   ```bash
   git add data/processed/hourly_workers.parquet
   git add output/data/intermediate_results/population/
   git add output/data/intermediate_results/individual_schedules/
...
```bash
git ls-files output/data/intermediate_results/population/
```
...
```bash
git ls-files output/data/intermediate_results/individual_schedules/ | wc -l
```
...
git ls-files data/processed/hourly_workers.parquet
