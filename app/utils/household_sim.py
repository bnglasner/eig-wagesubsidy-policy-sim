...
Two execution modes:
  1. Pre-computed (fast, deployment-ready): load parquet schedules generated by
     code/01_data_preparation/01b_precompute_individual.py, then
...
# ── Wage subsidy hours cap (policy design) ────────────────────────────────────
# Subsidy eligibility is capped at 40 hrs/week per job.
# Hours above this threshold are employer overtime and ineligible for the subsidy.
SUBSIDY_HOURS_CAP_PER_WEEK: int = 40
SUBSIDY_HOURS_CAP_ANNUAL: int = SUBSIDY_HOURS_CAP_PER_WEEK * 52  # 2,080 hrs/yr

# ── Overtime deduction parameters (OBBBA 2025, effective 2026) ────────────────
# Workers who are FLSA-overtime-eligible and work > 40 hrs/week can deduct the
# overtime premium (0.5× employer_wage × OT_hours × 52) from federal taxable income,
# subject to a cap and phase-out far above EIG-eligible worker incomes.
OVERTIME_DEDUCTION_CAP_SINGLE: float = 12_500.0   # single / HH / SS / separate
OVERTIME_DEDUCTION_CAP_JOINT:  float = 25_000.0   # married filing jointly
# Phase-out: starts at $150K (single) / $300K (joint), rate 10%.
# All EIG-eligible workers earn below $52K, so no phase-out applies — we skip it.
...
def _build_situation(
    ...
    family_type: str,
    state_code: str,
    hours_worked_last_week: float | None = None,
    is_paid_hourly: bool = True,
) -> dict:
    """
    Build a PolicyEngine situation dict for a stylised household.

    hours_worked_last_week: if provided, enables the OBBBA overtime income
        deduction for workers with > 40 hrs/week. All EIG-eligible workers are
        hourly (is_paid_hourly=True), making them FLSA-overtime-eligible.
    """
    ...
    person1: dict = {
        ...
        "is_paid_hourly":    {YEAR_STR: is_paid_hourly},
    }
    if hours_worked_last_week is not None:
        person1["hours_worked_last_week"] = {YEAR_STR: float(hours_worked_last_week)}
...
def compute_income_point(
    ...
    Pass hours_worked_last_week to enable the OBBBA overtime deduction for
    workers who work more than 40 hours per week. All EIG-eligible workers are
    hourly (is_paid_hourly=True by default).
    """
    try:
        from policyengine_us import Simulation
    except ImportError as exc:
    ...
    situation = _build_situation(
        annual_income, family_type, state_code,
