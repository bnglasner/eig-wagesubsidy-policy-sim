...
3. `.codex/commands/literature-intake.md` — the canonical catalog schema and intake playbook you persist into.
4. `.codex/rules/style-citation-rules.md` — EIG citation format for any public-facing output.
5. `.codex/rules/performance-cost-governance.md` — apply before a thorough multi-wave sweep.
...
- **Collect, do not format.** Capture complete, verified bibliographic data. Leave final public-facing formatting to `/cite` and `.codex/rules/style-citation-rules.md` — EIG format does not parenthesize the year, puts article titles in quotation marks, italicizes report/book titles, and ends every citation with a period. Do not let an academic house format leak into EIG deliverables.
...
- Leave EIG public-facing display formatting to `/cite` and `.codex/rules/style-citation-rules.md`; the BibTeX is the machine-readable record of truth, not the final rendered citation.
...
Persist findings using the canonical playbook in `.codex/commands/literature-intake.md`. Do not invent your own schema.
...
A thorough multi-wave snowball is a Tier 2–3 job under `.codex/rules/performance-cost-governance.md`. Before a thorough sweep, state the expected scope (number of seed scholars, snowball depth) and let the human pick **quick map** vs. **thorough sweep**. Default to a quick map when the request is exploratory.
